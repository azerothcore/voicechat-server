...
void status_updater_thread(void* unused)
{
    ...
    while( running )
    {
        ...
        // dump to console
        printf("\r| %02u servers | %05u channels | %05u users | in: %04.2f KB/s | out: %04.2f KB/s |",
            servers, channels, users, bandwidth_in, bandwidth_out );
...
int main(int argc, char* argv[])
{
    ...
    // read config file
    if( configfile_load_config("voicechat-server.conf") < 0 )
    {
