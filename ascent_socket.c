...
volatile int g_userCount = 0;

int get_server_count() { return g_serverCount; }
int get_channel_count() { return g_channelCount; }
int get_user_count() { return g_userCount; }
...
{
    int count = voice_remove_channels((void*)s);
    --g_serverCount;
    if (g_serverCount == 0)
    {
        g_userCount = 0;
...
void ascentsocket_send(ascent_socket *s, ascent_packet *p)
{
	...
	*(uint16*)(&buffer[2]) = p->size;
	...
	log_write(DEBUG, "send opcode (%u)", p->opcode);
}
...
int voicechat_ascent_listen_socket_read_handler(network_socket *s, int act)
{
	...
	if( new_fd < 0 )
	{
		log_write(ERROR, "FATAL: accept() returned < oops %d", new_fd);
		abort();
		return -1;
	}
...
void vc_handler_deletechannel(ascent_socket *s, ascent_packet *p)
{
	...
	channel_type = ascentpacket_readu8(p);
...
void vc_handler_addmember(ascent_socket *s, ascent_packet *p)
{
	...
	ch->members[member_id].enabled = 1;
	ch->members[member_id].voiced = 0;
	ch->members[member_id].muted = 0;
}
...
void vc_handler_deletemember(ascent_socket *s, ascent_packet *p)
{
    ...
    if (ch->members[member_id].voiced == 1)
        --g_userCount;

	ch->members[member_id].enabled = 0;
	ch->members[member_id].voiced = 0;
	ch->members[member_id].muted = 0;
}

void vc_handler_voicemember(ascent_socket* s, ascent_packet* p)
{
	uint16 channel_id;
	uint8 member_id;
	voice_channel* ch;
	...
	if (ch == NULL)
	{
		log_write(ERROR, "client is requesting us to voice a member from a nonexistant channel");
		return;
	}
	...
	if (ch->members[member_id].enabled == 0)
	{
		log_write(ERROR, "channel id %u slot %u is not enabled!");
		return;
	}

	log_write(DEBUG, "channel id %u slot %u is now voiced.", (int)channel_id, (int)member_id);
	ch->members[member_id].voiced = 1;
    ++g_userCount;
}

void vc_handler_devoicemember(ascent_socket* s, ascent_packet* p)
{
	uint16 channel_id;
	uint8 member_id;
	voice_channel* ch;
	...
	if (ch == NULL)
	{
		log_write(ERROR, "client is requesting us to devoice a member from a nonexistant channel");
		return;
	}
	...
	log_write(DEBUG, "channel id %u slot %u is now devoiced.", (int)channel_id, (int)member_id);
	ch->members[member_id].voiced = 0;
    --g_userCount;
}

void vc_handler_mutemember(ascent_socket* s, ascent_packet* p)
{
	uint16 channel_id;
	uint8 member_id;
	voice_channel* ch;
	...
	if (ch == NULL)
	{
		log_write(ERROR, "client is requesting us to mute a member from a nonexistant channel");
		return;
	}
	...
	log_write(DEBUG, "channel id %u slot %u is now muted.", (int)channel_id, (int)member_id);
	ch->members[member_id].muted = 1;
}

void vc_handler_unmutemember(ascent_socket* s, ascent_packet* p)
{
	uint16 channel_id;
	uint8 member_id;
	voice_channel* ch;
	...
	if (ch == NULL)
	{
		log_write(ERROR, "client is requesting us to unmute a member from a nonexistant channel");
		return;
	}
	...
	log_write(DEBUG, "channel id %u slot %u is now unmuted.", (int)channel_id, (int)member_id);
	ch->members[member_id].muted = 0;
}
